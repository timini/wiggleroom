...
struct SpectraHenge : FaustModule<VCVRackDSP> {
    ...
    void process(const ProcessArgs& args) override {
        ...
        // Audio I/O — 4 inputs, 2 outputs (stereo)
        float in1 = inputs[IN1_INPUT].getVoltage() * 0.2f;
        float in2 = inputs[IN2_INPUT].getVoltage() * 0.2f;
        float in3 = inputs[IN3_INPUT].getVoltage() * 0.2f;
        float in4 = inputs[IN4_INPUT].getVoltage() * 0.2f;
...
struct SpectraHengeWidget : ModuleWidget {
    SpectraHengeWidget(SpectraHenge* module) {
        ...
        // --- Q Knob below display ---
        float qRow = displayTop + displaySize + 18.f;
        addParam(createParamCentered<RoundBlackKnob>(
            Vec(centerX, qRow), module, SpectraHenge::Q_PARAM));

        // --- Jack layout ---
        // 4 columns for the 4 channels, plus extra positions for LFO/stereo out
        float col1 = 28.f;
        float col2 = 68.f;
        float col3 = box.size.x - 68.f;
        float col4 = box.size.x - 28.f;

        // Row 1: Audio inputs (IN 1-4)
        float jackRow1 = qRow + 40.f;
        ...
        // Row 2: X CV inputs
        float jackRow2 = jackRow1 + 34.f;
        addInput(createInputCentered<PJ301MPort>(Vec(col1, jackRow2), module, SpectraHenge::CV_X1_INPUT));
        addInput(createInputCentered<PJ301MPort>(Vec(col2, jackRow2), module, SpectraHenge::CV_X2_INPUT));
        addInput(createInputCentered<PJ301MPort>(Vec(col3, jackRow2), module, SpectraHenge::CV_X3_INPUT));
        addInput(createInputCentered<PJ301MPort>(Vec(col4, jackRow2), module, SpectraHenge::CV_X4_INPUT));

        // Row 3: Y CV inputs
        float jackRow3 = jackRow2 + 34.f;
        addInput(createInputCentered<PJ301MPort>(Vec(col1, jackRow3), module, SpectraHenge::CV_Y1_INPUT));
        addInput(createInputCentered<PJ301MPort>(Vec(col2, jackRow3), module, SpectraHenge::CV_Y2_INPUT));
        addInput(createInputCentered<PJ301MPort>(Vec(col3, jackRow3), module, SpectraHenge::CV_Y3_INPUT));
        addInput(createInputCentered<PJ301MPort>(Vec(col4, jackRow3), module, SpectraHenge::CV_Y4_INPUT));

        // Row 4: LFO A, LFO B, Output L, Output R
        float jackRow4 = jackRow3 + 34.f;
        addInput(createInputCentered<PJ301MPort>(Vec(col1, jackRow4), module, SpectraHenge::LFO_A_INPUT));
        addInput(createInputCentered<PJ301MPort>(Vec(col2, jackRow4), module, SpectraHenge::LFO_B_INPUT));
        addOutput(createOutputCentered<PJ301MPort>(Vec(col3, jackRow4), module, SpectraHenge::OUT_L_OUTPUT));
        addOutput(createOutputCentered<PJ301MPort>(Vec(col4, jackRow4), module, SpectraHenge::OUT_R_OUTPUT));
    }
};
