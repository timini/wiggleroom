 ...
 * Vintage tri-phase chorus for massive orchestral strings
...
#include "tri_phase_ensemble.hpp"  // Generated by Faust
...
/**
 * TriPhaseEnsemble - Classic String Ensemble Effect
 *
 ...
 *   - Rate: LFO speed scaling
 *   - Tone: BBD lowpass filter frequency
 *   - Mix: Dry/wet blend
 */
struct TriPhaseEnsemble : FaustModule<VCVRackDSP> {
    enum ParamId {
        DEPTH_PARAM,
        RATE_PARAM,
        TONE_PARAM,
...
struct TriPhaseEnsembleWidget : ModuleWidget {
    TriPhaseEnsembleWidget(TriPhaseEnsemble* module) {
        setModule(module);
        box.size = Vec(4 * RACK_GRID_WIDTH, RACK_GRID_HEIGHT);
        addChild(new WiggleRoom::ImagePanel(
            asset::plugin(pluginInstance, "res/TriPhaseEnsemble.png"), box.size));
        ...
        addParam(createParamCentered<RoundBigBlackKnob>(
            Vec(xCenter, 55), module, TriPhaseEnsemble::DEPTH_PARAM));

        // Rate and Tone knobs
        addParam(createParamCentered<RoundBlackKnob>(
            Vec(xLeft, 110), module, TriPhaseEnsemble::RATE_PARAM));
        addParam(createParamCentered<RoundBlackKnob>(
            Vec(xRight, 110), module, TriPhaseEnsemble::TONE_PARAM));

        // Mix knob
        addParam(createParamCentered<RoundBlackKnob>(
            Vec(xCenter, 160), module, TriPhaseEnsemble::MIX_PARAM));

        // Mix CV input
        addInput(createInputCentered<PJ301MPort>(
            Vec(xCenter, 200), module, TriPhaseEnsemble::MIX_CV_INPUT));

        // Audio inputs
        addInput(createInputCentered<PJ301MPort>(
            Vec(xLeft, 265), module, TriPhaseEnsemble::LEFT_INPUT));
        addInput(createInputCentered<PJ301MPort>(
            Vec(xRight, 265), module, TriPhaseEnsemble::RIGHT_INPUT));

        // Stereo Outputs
        addOutput(createOutputCentered<PJ301MPort>(
            Vec(xLeft, 320), module, TriPhaseEnsemble::LEFT_OUTPUT));
        addOutput(createOutputCentered<PJ301MPort>(
            Vec(xRight, 320), module, TriPhaseEnsemble::RIGHT_OUTPUT));
    }
};

} // namespace WiggleRoom

Model* modelTriPhaseEnsemble = createModel<WiggleRoom::TriPhaseEnsemble, WiggleRoom::TriPhaseEnsembleWidget>("TriPhaseEnsemble");
