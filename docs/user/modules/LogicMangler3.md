...
LogicMangler3 is a WiggleRoom module in the **Sequencers & Clocks** category. A 3-input truth table logic processor with cell locking, density control and randomisation. It works standalone or as an expander for [EucSeq3](EucSeq3.md).

It runs the same truth table engine as the 4-channel [LogicMangler](LogicMangler.md), sized for 3 gate streams. With 3 inputs the table has 8 rows, one per input combination.

## Signal Flow

- LogicMangler3 processes gates and triggers, not audio.
- Inputs accept 3 Gate signals plus per-channel Probability CV.
- Outputs emit a processed Gate and Trigger per channel.
- Placed to the right of EucSeq3 it picks up that module's gates over the expander connection, so the Gate inputs can be left unpatched.
